/*
 * AquaLevel Pro - ESP32 Version (Power Optimized)
 * Removed SIM800L functionality for power saving
 * Designed by: Komkrit Chooraung
 * Date: 28-Nov-2025
 * Version: 3.2 (Power Optimized - No Modem)
 * Added: Deep sleep with 15min active window + 20min sleep
 * Added: AXP192 battery monitoring for TTGO SIM800L
 * GPIO: RX=19, TX=18, ForceAP=35, LEDs: 12,14,15
 * MODEM: COMPLETELY DISABLED for power saving
 */
...
// --- SIM800L MODEM DISABLED - POWER SAVING ---
#define MODEM_PWRKEY 4      // GPIO4 - Modem power key
#define MODEM_POWER_ON 23   // GPIO23 - Modem power enable
#define MODEM_RST 5         // GPIO5 - Modem reset
...
#define FORCE_AP_PIN 27  // GPIO27 force AP mode
...
void readVBUS() {
  ...
  if (axp.isBatteryConnect()) {
    ...
    // To display the charging status
    if (axp.isChargeing()) {
      Serial.print("Charge:");
...
void initAXP192() {
  ...
  axp.adc1Enable(AXP202_VBUS_VOL_ADC1 | AXP202_VBUS_CUR_ADC1 | AXP202_BATT_CUR_ADC1 | AXP202_BATT_VOL_ADC1,
                 true);
...
void disableModemPower() {
  // This is the KEY POWER SAVING FUNCTION
  // Turn off the SIM800L modem completely to save power
  
  Serial.println("=== DISABLING SIM800L MODEM FOR POWER SAVING ===");
  
  // Configure modem control pins as outputs
  pinMode(MODEM_PWRKEY, OUTPUT);
  pinMode(MODEM_POWER_ON, OUTPUT);
  pinMode(MODEM_RST, OUTPUT);
  
  // Turn off modem power
  digitalWrite(MODEM_POWER_ON, LOW);    // Disable modem power
  digitalWrite(MODEM_PWRKEY, LOW);      // Ensure modem is off
  digitalWrite(MODEM_RST, LOW);         // Keep reset low
  
  // Also disable the SIM800L UART TX pin to prevent power leakage
  pinMode(27, INPUT_PULLDOWN);  // GPIO27 is MODEM_TX on TTGO board
  
  Serial.println("SIM800L modem powered off completely");
  Serial.println("Expected power saving: ~1-2mA reduction");
  Serial.println("========================================");
}

void enableModemPower() {
  // Only needed if you want to use modem later
  pinMode(MODEM_PWRKEY, OUTPUT);
  pinMode(MODEM_POWER_ON, OUTPUT);
  pinMode(MODEM_RST, OUTPUT);
  ...
  Serial.println("SIM800L modem powered on");
}

void checkBattery() {
  // Read battery voltage
  batteryVoltage = axp.getBattVoltage() / 1000.0f;  // Convert mV to V
...
void processSerialCommand(String command) {
  ...
  } else if (command == "modem_on") {
    enableModemPower();
    Serial.println("Modem powered on (if you need it)");
  } else if (command == "modem_off") {
    disableModemPower();
    Serial.println("Modem powered off for power saving");
  } else if (command.length() > 0) {
    Serial.printf("Unknown command: '%s'\n", command.c_str());
    Serial.println("Available commands: mac, read, benchmark, reset, eeprom_reset, preferences_dump, sleep, sleep_disable, sleep_enable, battery, modem_on, modem_off");
  }
}
...
void setup() {
  ...
  Serial.println("===============================================");
  Serial.println("  POWER OPTIMIZED VERSION - MODEM DISABLED     ");
  Serial.println("===============================================");

  // ===== KEY POWER SAVING STEP =====
  // Disable SIM800L modem immediately to save power
  disableModemPower();

  // Initialize AXP192 battery management
  initAXP192();
  ...
  // Reduce WiFi power to minimum
  WiFi.setTxPower(WIFI_POWER_19_5dBm);  // ~19.5 dBm, lower than default
  ...
  Serial.println("1. SIM800L modem completely disabled");
  Serial.println("2. WiFi power reduced");
  Serial.println("3. Deep sleep enabled");
  blinkGreenLED(2);
}
